...
import { generateAccessToken, generateRefreshToken, setRefreshTokenCookie, verifyRefreshToken } from '../utils/authUtils.js';
...
export const signupUser = async (req, res) => {
    ...
    const [newUser] = await db.insert(UserTable).values({
        ...
        admin: isAdmin
    }).returning({ id: UserTable.id, email: UserTable.email, username: UserTable.username, admin: UserTable.admin });
    ...
    res.status(201).json({ message: 'User created successfully.', user: { email, username, admin: isAdmin }, accessToken});
}
